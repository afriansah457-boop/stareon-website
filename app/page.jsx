...
export default function StareonProDashboard() {
    ...
    useEffect(() => {
        if (session?.accessToken) {
            ...
            .then(res => res.json())
            .then(data => {
                setGuilds(data.filter(g => g.owner || (g.permissions & 0x8) === 0x8));
                ...
                <div className="glass-card">
                    {activeTab === 'welcome' && (
                        <div>
                            ...
                            <label className="input-label">ID Channel Welcome</label><input type="text" className="modern-input" placeholder="Misal: 123456789" />
                            <label className="input-label" style={{marginTop:'15px'}}>Pesan Welcome</label><textarea className="modern-input" rows="2" placeholder="Halo {user}!" />
                            <hr style={{borderColor:'#1e293b', margin:'20px 0'}}/>
                            <label className="input-label">ID Channel Goodbye</label><input type="text" className="modern-input" placeholder="Misal: 123456789" />
                            <label className="input-label" style={{marginTop:'15px'}}>Pesan Goodbye</label><textarea className="modern-input" rows="2" placeholder="Sampai jumpa {user}." />
                        </div>
                    )}
                    {activeTab === 'antilink' && (
                        <div>
                            <h2 className="tab-title"><Shield size={20}/> Anti-Link System</h2>
                            <p className="tab-desc">Hapus otomatis jika member spam link 2x berturut-turut.</p>
                            <div className="toggle-box"><span>Aktifkan Anti-Link</span><input type="checkbox" defaultChecked style={{width:'20px',height:'20px'}}/></div>
                        </div>
                    )}
                    {activeTab === 'leveling' && (
                        <div>
                    ...
                    {activeTab === 'ticket' && (
                        <div>
                            ...
                            <button className="btn-action-pro" style={{background:'#6366f1'}}>KIRIM PANEL TIKET</button>
                        </div>
                    )}
